function dpd_out = tx_dfe(tx_in, config)
    ...
    % נרמול RMS (Gain Factor Alignment)
    gain_factor = sqrt(mean(abs(pa_distorted_initial).^2)) / sqrt(mean(abs(x_in_train).^2));
    y_out_norm = pa_distorted_initial / gain_factor;

    % אימון ה-DPD (החזרת אובייקט מעודכן עם Coeffs)
    dpd_model = dpd_model.train(x_in_train, y_out_norm);
